...
ESP_AVRISP::ESP_AVRISP(uint16_t port, uint8_t cipo, uint8_t copi, uint8_t clk,
                       uint8_t reset_pin, uint32_t spi_freq, bool reset_state,
...
void ESP_AVRISP::begin() { _server.begin(); }
  ...
  switch (_state)
  {
    case AVRISP_STATE_IDLE:
    {
      if (_server.hasClient())
      {
        ...
        AVRISP_DEBUG("client connect %d.%d.%d.%d:%d", IP2STR(&lip),
                     _client.remotePort());
